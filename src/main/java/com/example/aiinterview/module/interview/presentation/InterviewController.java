...
public class InterviewController {
    ...
    public Mono<List<InterviewSession>> retrieveInterviewSession(@RequestAttribute("userId") Long memberId){
        return applicationService.retrieveInterviewSession(memberId);
    }

    @PostMapping()
    @Operation(summary = "인터뷰 세션 생성(토큰 필요)", description = "토큰을 포함해야 하며, 인터뷰 세션(방) 을 생성합니다.")
    public Mono<InterviewSession> createSession(@RequestAttribute("userId") Long memberId) {
        return applicationService.createRoom(memberId);
    }

    @PostMapping("/{sessionId}/complete")
    @Operation(summary = "인터뷰 세션을 종료", description = "토큰을 포함해야하며, 인터뷰 세션을 종료합니다.")
    public Mono<Void> completeSession(
    ...
    public Mono<List<InterviewMessage>> retrieveMessage(@Parameter(description = "세션 고유 ID") @PathVariable Long sessionId,
...
//                                         @RequestHeader("Last-Event-ID") String lastEvnetId,
    ...
    public Mono<String> retrieveMessageHistory(@Parameter(description = "세션 고유 ID") @PathVariable Long sessionId,
                                               @RequestAttribute("userId") Long memberId){
